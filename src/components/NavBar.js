...
const NavBar = () => {
  ...
  return (
    <header className="w-full px-32 py-8 font-medium flex items-start justify-between dark:text-light relative z-10 lg:px-16 md:px-12 sm:px-8 ">
      {/* hambuger menu */}
      ...
      {/* Mobile View Navbar Content */}
      {
        ...
        animate={{scale:1,opacity:1}} className="min-w-[70vw] z-30 bg-dark/90 dark:bg-light/75 rounded-lg backdrop-blur-md py-32 flex flex-col justify-between items-center fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
      ...
      <nav className="flex items-center justify-center flex-wrap ">
        ...
        <button onClick={() => { setmode(mode==="light"?"dark":"light")}} className="ml-4 sm:mx-1 flex items-center justify-center rounded-full p-1 dark:bg-light dark:text-dark">
          {
            mode === "dark" ?
            <SunIcon className={"fill-dark"}/> :
            <MoonIcon className={"fill-dark"}/>
