...
function Card({src,title,key_techs,description,github,link}) {
    return (
      <>
       <div className={`p-4 flex flex-col w-[400px] h-[500px] rounded-lg border-2 border-solid border-dark dark:border-dark bg-gradient-to-b  from-[#000000] via-[#868B8C] to-[#EAF1F1] `}>
       <div className="absolute center top-36 -left-2 -z-10 w-[102%]
      h-[80%] rounded-[2rem] bg-dark  "/>
            <Image src={src} alt="img" className="w-full h-[50%]  hover:scale-105 duration-300"/>
          ...
          <Link href={link} target="_blank" className="ml-4 rounded-lg  p-2 px-6 text-lg font-semibold border-2 border-solid border-transparent bg-dark text-light hover:text-dark hover:border-dark hover:bg-light
              ">
            Visit Project
          </Link>
        </div>
